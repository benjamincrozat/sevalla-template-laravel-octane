...
# - Use Alpine + PHP-FPM with Nginx, supervised by Supervisord
...
# 3. runtime: Final image (php-fpm + nginx + supervisor) with artifacts copied in
...
# Provide a simple Nginx config serving `public/` on port 8080 and forwarding PHP to PHP-FPM
...
# Configure Supervisord to run PHP-FPM and nginx in the same container
...
# Entrypoint is expected to start Supervisord, which manages PHP-FPM and Nginx
